...
> Package boundary and coding rules for `psymoth-core` and the CLI.
...
|---|---|
| `psymoth-core` | Reusable domain logic, storage primitives, reducers, task APIs, channel APIs, and typed contracts. |
| `psymoth` | CLI argument parsing, terminal rendering, command wiring, process exit behavior, template installation, migrations, and release scripts. |
...
```ts
import { createChannelStore } from "psymoth-core/channel";
```
...
```bash
pnpm --filter psymoth-core build
