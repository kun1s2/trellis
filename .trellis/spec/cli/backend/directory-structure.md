...
This project is a **TypeScript monorepo** using ES modules. It publishes a CLI package (`psymoth`) and a reusable core package (`psymoth-core`). The source code also follows a **dogfooding architecture** - Trellis uses its own configuration files (`.cursor/`, `.claude/`, `.trellis/`) as templates for new projects.
...
```
packages/
├── core/                # psymoth-core: reusable APIs
│   ├── src/
