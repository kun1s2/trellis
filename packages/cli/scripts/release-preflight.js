 ...
 *   1. Version match between `@kun/trellis` and
 *      `@kun/trellis-core` (and the current git tag when checked from
 *      a tag context).
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["@kun/trellis-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on @kun/trellis-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on @kun/trellis-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins @kun/trellis-core to exact ${v.cliVersion}.`,
    );
  } finally {
