 ...
 *   1. Version match between `psymoth` and
 *      `psymoth-core` (and the current git tag when checked from
 *      a tag context).
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["psymoth-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on psymoth-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on psymoth-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins psymoth-core to exact ${v.cliVersion}.`,
    );
  } finally {
