...
describe("init() integration", () => {
  ...
  it("#10b localizes human-readable docs generated by default init", async () => {
    await init({ yes: true, user: "dev" });

    const generatedDocs = [
      {
        label: "AGENTS.md",
        file: path.join(tmpDir, FILE_NAMES.AGENTS),
        phrase: "默认使用中文",
      },
      {
        label: ".trellis/workflow.md",
        file: path.join(tmpDir, PATHS.WORKFLOW_GUIDE_FILE),
        phrase: "面向人类阅读的 Trellis 文档",
      },
      {
        label: ".trellis/spec/backend/index.md",
        file: path.join(tmpDir, PATHS.SPEC, "backend", "index.md"),
        phrase: "默认使用**中文**编写",
      },
      {
        label: ".trellis/spec/frontend/index.md",
        file: path.join(tmpDir, PATHS.SPEC, "frontend", "index.md"),
        phrase: "默认使用**中文**编写",
      },
      {
        label: ".trellis/workspace/index.md",
        file: path.join(tmpDir, PATHS.WORKSPACE, "index.md"),
        phrase: "默认使用**中文**编写",
      },
    ] as const;
    ...
    for (const doc of generatedDocs) {
      ...
      expect(content, `${doc.label} should state Chinese default`).toContain(
        doc.phrase,
      );
      expect(content, `${doc.label} should preserve technical terms`).toContain(
        "technical terms",
      );
      expect(content, `${doc.label} should not keep old English rule`).not.toMatch(
    ...
    expect(bootstrapPrd).toContain("默认用中文交流");
    expect(bootstrapPrd).toMatch(/English technical\s+terms/);
    expect(bootstrapPrd).toContain("`trellis init`");
    expect(bootstrapPrd).toContain("`trellis-implement`");
    expect(bootstrapPrd).toContain("`trellis-check`");
    expect(bootstrapPrd).toContain("`implement.jsonl`");
    expect(bootstrapPrd).toContain("`check.jsonl`");
    expect(bootstrapPrd).not.toMatch(OLD_ENGLISH_DOCUMENTATION_RULE);
  });
  ...
  it("#15 monorepo: bootstrap task references per-package spec paths", async () => {
    ...
    expect(prd).toContain("- [ ] 填写 core 的 guidelines");
    expect(prd).toContain("- [ ] 填写 ui 的 guidelines");
    expect(prd).toContain("默认用中文交流");
    expect(prd).toMatch(/English technical\s+terms/);
    expect(prd).not.toMatch(OLD_ENGLISH_DOCUMENTATION_RULE);
