...
Use a conservative default: if you cannot prove the skip criteria from repository/task evidence, choose a grill path. Use `grill-me` / `trellis-grill-me` when real user decisions remain. Use `trellis-grill-agents` only when the user explicitly authorized unattended/proxy answers; if it exposes a real user decision, stop and route back to the user.
